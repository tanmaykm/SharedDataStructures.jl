...
function test_shm_dict(keepkeys::Bool)
    @testset "byte representation" begin
        for T in (Float64, Float32, Int64, Int32, Int16, Complex{Int64}, Complex{Int32})
            v = rand(T)
            @test v == SharedDataStructures._from_byte_repr(T, SharedDataStructures._byte_repr(v))
        end
        v = rand(UInt8, 5)
        @test v == SharedDataStructures._from_byte_repr(Vector{UInt8}, SharedDataStructures._byte_repr(v))
        v = (1,2,3)
        @test v == SharedDataStructures._from_byte_repr(Any, SharedDataStructures._byte_repr(v))
    end
